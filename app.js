...
const ZOOM_PONTOS = 15;     // a partir daqui, árvore e poste um a um
...
function pintarCartoes(onde, base, props) {
  ...
  $("#cartoes").innerHTML = `
    ...
    <dl>
      ...
      <dt><i class="marca-serie" style="background:var(--serie-1)"></i>estreita</dt><dd>${pct(c.estreita)}</dd>
      <dt><i class="marca-serie" style="background:var(--serie-2)"></i>íngreme</dt><dd>${pct(c.ingreme)}</dd>
...
function pintarCartoesCalcada(nome, mostradas, total) {
  ...
  $("#cartoes").innerHTML = `
    ...
    <dl>
      ...
      <dt><i class="marca-serie" style="background:var(--serie-1)"></i>estreita</dt><dd>${pct(parte(p => p.livre_min < FAIXA_LIVRE_MIN))}</dd>
      <dt><i class="marca-serie" style="background:var(--serie-2)"></i>íngreme</dt><dd>${pct(parte(p => p.declive > DECLIVIDADE_MAX))}</dd>
...
function atualizarPorVista() {
  ...
  camadaDistritos.eachLayer(l => {
    if (b.intersects(l.getBounds())) visiveis.push(l.feature.properties);
  });
...
async function abrirDistrito(props) {
  const id = props.id;
  if (!emCache.has(id)) {
  ...
  montarCalcadas();
  if (camadaDistritos) mapa.removeLayer(camadaDistritos);
  mapa.fitBounds(camadaCalcadas.getBounds(), {padding: [24, 24], animate: false});
  desenharLegenda();
 ...
 * únicas camadas de ponto em disco. Fora dali os botões ficam desligados. */
...
function desenharTabela() {
  const m = METRICAS[metrica];
  const fs = distritos.features.map(f => f.properties)
    .filter(p => valorDe(p, m) != null)
    .sort((a, b) => m.pior === "baixo" ? valorDe(a, m) - valorDe(b, m)
                                       : valorDe(b, m) - valorDe(a, m));
  $("#tabela").innerHTML = `
    <table class="tab">
      <thead><tr><th scope="col">distrito</th><th scope="col">${m.rot}</th></tr></thead>
      <tbody>${fs.map(p => {
        const v = valorDe(p, m);
...
function desenharControles() {
  $("#controles").innerHTML = `
    ...
    <div class="grupo"><span>camadas do piloto</span><div class="pills">
      <button id="lay-arvores" aria-pressed="true" disabled>árvores</button>
      <button id="lay-postes" aria-pressed="true" disabled>postes</button>
    </div></div>
...
function desenharLegenda() {
  const m = METRICAS[metrica];
  const passos = m.pior === "baixo" ? [...RAMPA].reverse() : RAMPA;
  if (distritoAberto) {
    $("#legenda").innerHTML = `
  ...
  $("#legenda").innerHTML = `
    ...
    <div class="escala-rot"><span>0%</span><span>mais escuro = pior</span><span>${m.max}%</span></div>
...
function pintarFaixa() {
  ...
  $("#vao").innerHTML = `
    ...
    <div class="lado"><dt><i class="marca-serie" style="background:var(--serie-1)"></i>estreita</dt>
      <dd>${pct(c.estreita)}<small>faixa livre abaixo de 1,20 m, já descontando árvore e poste</small></dd></div>
    <div class="lado"><dt><i class="marca-serie" style="background:var(--serie-2)"></i>com obstáculo</dt>
...
function autoteste() {
  ...
  barra.style.cssText = "position:fixed;left:0;right:0;bottom:0;z-index:9999;padding:9px 14px;" +
    ...
    (falhas.length ? "#8f2a17" : "#14532d");
  barra.textContent = falhas.length
    ? "autoteste: " + falhas.map(([r, a, b]) => `${r} ${a.toFixed(2)} ≠ ${b}`).join(" · ")
...
(async function () {
  ...
  mapa.on("moveend zoomend", adiar(() => { atualizarPorVista(); alternarPontos(); }));

  // Link direto para um distrito, com os filtros já ligados:
  //   #d=grajau&f=larga,livre
